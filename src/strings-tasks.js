...
function formatTime(minutes, seconds) {
  ...
  const tempMin = String(minutes).padStart(2, '0');
  const tempSeconds = String(seconds).padStart(2, '0');
  return `${tempMin}:${tempSeconds}`;
}
...
function isPalindrome(str) {
  ...
  const temp = str.toLowerCase().replaceAll(/[\W_]/g, '');
  let start = 0;
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < temp.length; i += 1) {
    const charCode = temp[i].charCodeAt(0);
    if (charCode >= 65 && charCode <= 90) {
      temp[i] = String.fromCharCode(((charCode - 65 + 13) % 26) + 65);
    } else if (charCode >= 97 && charCode <= 122) {
      temp[i] = String.fromCharCode(((charCode - 97 + 13) % 26) + 97);
    }
  }
  return temp.join('');
}
...
function getCardId(value) {
  ...
  return cardsValue.indexOf(value);
}
